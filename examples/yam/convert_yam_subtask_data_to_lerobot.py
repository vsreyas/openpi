"""
Script to convert YAM teleop data with sub-task language annotations to LeRobot dataset format.
...
def convert(
    ...
    """Convert YAM teleop data with sub-task annotations to LeRobot format.
    ...
    with ThreadPoolExecutor(max_workers=num_preload_workers) as pool:
        ...
        for ep_dir, future in tqdm.tqdm(zip(episode_dirs, futures), total=len(futures), desc="Converting"):
            ...
            print(f"  {ep_dir.name}: {len(segments)} sub-tasks written")
